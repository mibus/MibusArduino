...
void lcdHandler(void) {
  ...
  if (! lcdInitialized) {
    lcdInitialize();
    lcdClear();
    lcdWriteString("Clock");
    lcdPosition(2,0);
    lcdWriteString("mibus@mibus.org");
  }

  lcdPosition(0, 12);

  if (flashThing == 1 && flashToggle) {
    lcdWriteString("XX");
  } else {
