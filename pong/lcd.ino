...
void clearThing (int x,int y) {
  lcdPosition (y, x);
  lcdWrite(' ',1);
}

void drawThing (int x, int y, int type) {
  lcdPosition (y, x);
  if (type == 0)
    lcdWrite('O',1);
  else
...
void pongMove(int dir) {
  if (millis()-last_move < 50)
    return;
  last_move = millis();
  clearThing(19,pongPos);
  pongPos+=dir;
  if (pongPos<0)
    pongPos=0;
  if (pongPos>3)
...
//  char foobar[40]="";
//  sprintf(foobar, "Pos: %d, %d; Vel: %d, %d; Paddle; %d\r\n", ball_x,ball_y,ball_v_x,ball_v_y, pongPos);
//  Serial.print (foobar);
  drawThing(19,pongPos,1);
  clearThing(ball_x,ball_y);
  ball_x += ball_v_x;
  ball_y += ball_v_y;  
  if (ball_y < 0) {
    ball_y = -ball_y;
  ...
  if (ball_x < 1) {
    ...
    if (ball_y == pongPos) {
      ...
      lcdPosition (ball_y, ball_x);
      lcdWrite('X',1);
      while (1) {}
    }
  }
  drawThing(ball_x,ball_y,0);
